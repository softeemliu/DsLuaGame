...
CDFErrorCode checkbufferoverflow(bytestream* stream, int addLength) {
	...
	if (!newBuffer) return CDF_ERROR_OUT_OF_MEMORY;
...
CDFErrorCode bytestream_append(bytestream* stream, const void* data, int dataSize) {
	if (checkbufferoverflow(stream, dataSize) != CDF_SUCCESS) return CDF_ERROR_OUT_OF_MEMORY;
...
void bytestream_clear(bytestream* stream) {
	...
	if (stream->bufferSize > CDF_BYTES_BUFFER_MAX_SIZE) {
		free(stream->buffer);
		stream->buffer = NULL;
		stream->bufferSize = 0;
	}
...
CDFErrorCode bytestream_writebytearray(bytestream* stream, const byte* data, int dataSize) {
	...
	if (checkbufferoverflow(stream, dataSize) != CDF_SUCCESS)
		return CDF_ERROR_OUT_OF_MEMORY;
...
// 从缓冲区头部移除指定长度的数据
void bytestream_popdata(bytestream* stream, int length) {
	if (stream->readOnly) {
		THROW_EXCEPTION("Attempt to pop data from read-only stream");
		return;
	}
	...
	// 1. 计算剩余数据量和移动位置
	int remainingData = stream->dataSize - length;

	// 2. 移动剩余数据到缓冲区头部
	memmove(stream->buffer, stream->buffer + length, remainingData);

	// 3. 更新数据大小
	stream->dataSize = remainingData;

	// 4. 调整读取索引
	if (stream->readIndex < length) {
		// 如果读取位置在移除范围内，重置为0
		stream->readIndex = 0;
	}
	else {
